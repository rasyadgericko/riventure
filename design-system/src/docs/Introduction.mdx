...
The design language behind [rycworks.com](https://www.rycworks.com) — extracted, refined, and documented for consistent builds across every project.

---

## How to Use This Guide

This design system serves two audiences. Toggle between them using the button at the bottom:

### Human Mode
Visual previews, design rationale, usage context, and do/don't guidelines. Built for designers, founders, and stakeholders who need to understand *why* a decision was made.

### Machine Mode
A single structured text document encoding the complete design system. Built for AI tools that need to generate on-brand code and copy.
...
|---------|---------------|
| [Colors](?path=/docs/foundations-colors--docs) | Core palette, semantic colors, theme tokens |
| [Typography](?path=/docs/foundations-typography--docs) | Type scale, font families, line heights |
| [Spacing](?path=/docs/foundations-spacing--docs) | 4px grid system, spacing scale |
| [Shadows](?path=/docs/foundations-shadows--docs) | Elevation levels, box shadow tokens |

## Components

| Component | Variants |
|-----------|----------|
| [Button](?path=/docs/components-button--docs) | Primary, Outline, Submit |
| [Tag](?path=/docs/components-tag--docs) | Default, Featured, AI |
| [Input](?path=/docs/components-input--docs) | Text, Email, with error |
| [Select](?path=/docs/components-select--docs) | Default, with placeholder |
| [Textarea](?path=/docs/components-textarea--docs) | Default |
| [TextLink](?path=/docs/components-textlink--docs) | Animated underline |
...
- **Theme**: Light `#f1f1f1` / Dark `#0f0f0f` with automatic switching
- **Radius**: Pill `100px` for buttons and tags, `8px` for cards
- **Motion**: `cubic-bezier(0.16, 1, 0.3, 1)` — smooth, bouncy exit
- **Grid**: 4px base unit, section padding `8rem 4vw`

</HumanOnly>

<MachineOnly>

<MachineText>{`name: ryc-brand
...
- CSS grid animation for smooth height transition`}</MachineText>
